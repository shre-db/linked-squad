...
class ProfileBotState(BaseModel):
    ...
    current_user_instructions: Optional[Dict[str, Any]] = Field(None, description="Structured instructions from the user for the current task, including summary, confidence, and raw data from LLM-based extraction.")
    conversation_context: Optional[str] = Field(None, description="Contextual information about the ongoing conversation to maintain continuity.")
    user_preferences: Dict[str, Any] = Field(default_factory=dict, description="User preferences learned from conversation (tone, style, focus areas).")
    ...
    pending_agent_output: Optional[Dict[str, Any]] = Field(None, description="Raw output from specialized agent waiting for router processing.")
    needs_output_processing: bool = Field(False, description="Flag: True if agent output needs router interpretation before user response.")
